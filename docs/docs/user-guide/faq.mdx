...
Yes, but not simultaneously in the same browser/app. You need to:
1. Log out of current account
2. Log in with different account

Consider using different browser profiles for multiple accounts on web.

### What happens to my AREAs if I delete my account?

⚠️ **Warning**: Account deletion is permanent and will:
- Delete all your AREAs
- Remove all execution history
- Disconnect all services
- Cannot be undone

Export your data first if you need to keep records.

### How secure is my data?

AREA takes security seriously:
- 🔐 OAuth authentication (we never store your service passwords)
...
**Mobile:**
1. Long-press on AREA
2. Select "Duplicate"
3. Modify the copy

Great for creating variations of similar automations.

### What are dynamic variables?

Dynamic variables let you insert data from the action into your reaction.

**Syntax:** `{{variable.name}}`

**Example:**
```
Action: GitHub - New Issue
...
See [Available Services](/user-guide/services) for variables per action.
...
Export important history before it's deleted!
...
### Variables show as `{{variable.name}}` in output

This means the variable is not being replaced. Causes:

1. **Wrong variable name**: Check spelling and case
2. **Variable not available**: Not all actions provide all variables
3. **Service doesn't support variables**: Check service documentation

**Solution:** See [Available Services](/user-guide/services) for exact variable names per action.
...
Yes, the open-source version is completely free. Hosted versions may have different pricing tiers.

**Check your plan:**
- Profile → Settings → Subscription
- View current limits and features

### What are the free tier limits?

Varies by deployment:
- **Open-source (self-hosted)**: No limits (hardware dependent)
- **Hosted free tier**: Check website for current limits

### How do I upgrade?

If using a hosted version with paid tiers:
- Profile → Settings → Subscription
- Select desired plan
- Complete payment
...
4. All data will be deleted within 30 days

**Alternative:** Just stop using AREA - accounts are free.
